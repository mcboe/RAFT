...
class Rotor:
    ...
    def calcAero(self, case, current=False, display=0):
        ...
        # Plot
        plt.figure(figsize=(8, 5))
        plt.plot(self.w/2/np.pi, S_rot*2*np.pi, label='Kaimal Rotor-Averaged Spectrum')
        plt.plot(self.w/2/np.pi, U, label='Kaimal Spectrum')
        plt.xlabel('Frequency [Hz]')
        plt.ylabel(r'$S(f)$ [$\mathrm{(m/s)^2/Hz}$]')
        plt.title('Example Kaimal Turbulence Spectrum')
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.show()
